...
- **Wikidata (PUBLICADO, no ar, jul/2026):** item **Q140469163** criado pelo Ibson. Rótulo/descrição em pt-br/en/es + 10 declarações: instance of human, sex male, data de nascimento 1988-09-14 (pública, escolha dele), país Brasil, ocupação empresário (Q43845, com referência), empregador Better Collective (Q111843935, com referência), nome de nascimento, site oficial, X (IbsonJunior), LinkedIn (ibson-junior). Referências independentes (Yahoo Finance, Bolavip) em ocupação/empregador. **Já no `sameAs`** do site (`schema.ts`, commit 6761389). Ciclo fechado dos 2 lados: site → Wikidata (sameAs) e Wikidata → site (P856 official website). Ingestão pelo Google/IAs: semanas a meses, sem garantia de painel; pode ser patrulhado por notabilidade (por isso as referências). Guia usado: `docs/wikidata-item.md`.
...
- **Search Console:** reiniciar **"Validar correção"** do erro mainEntity (ProfilePage da home já corrigida, commit 8b41eb8; validei o JSON-LD ao vivo).
- **Bios:** colar LinkedIn (headline + Sobre) e X (bio + site) de `docs/bios-perfis.md`. Link ibsonjunior.com.br em ambos (recíproco com o `sameAs`). Mesma foto profissional em todos os perfis.
- ~~**Wikidata:** criar o item~~ **FEITO:** item Q140469163 no ar e já no `sameAs`. Nada pendente aqui.
